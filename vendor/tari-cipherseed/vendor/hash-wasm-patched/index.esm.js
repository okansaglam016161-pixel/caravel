...
const validateOptions$3 = (options) => {
    ...
    // PATCHED (tari-cipherseed): upstream hash-wasm additionally rejects a zero-length password
    // here (`options.password.length < 1`), but Argon2 itself defines an empty password as valid
    // input (RFC 9106) and argon2Internal() below never checks the length -- the rejection is a
    // defensive JS-side guard, not an algorithmic requirement. Tari's CipherSeed format allows an
    // explicitly empty seed passphrase, so that guard is removed here; every other validation
    // (salt length, iteration/parallelism/memory bounds, output type) is untouched.
